...
which_plots = [0,1,2,7,8,9,11,12,14,15,17]  # This one has all the "good" data
#which_plots = [0]
#which_plots = [1]
...
for i, plot_num in enumerate(which_plots):
    ...
    #        for k in rang:
    #            if k == j:
    #                if single_sweep_list[k] == 0:
    #                    single_sweep_list[k] = [val]
    #                else:
    #                    single_sweep_list[k].append(val)

                #if col == single_sweep_index:
                #    single_sweep_list.append(val)
...
comb_mat = np.full_like(collector_array_nested[0], 0)
for i, col_arr in enumerate(collector_array_nested):
#    comb_mat += col_arr
    print(len(col_arr), len(col_arr[0]))
    j = 0
    for row in col_arr:
        for col in row:
            if big_list[j] == 0:
...
#single_sweep_list_std = np.std(single_sweep_list)
#single_sweep_list_mean = np.mean(single_sweep_list)
...
ax1.set(title='Stats 4 - Mean vs. Sigma (Points)', xlabel='Mean', ylabel='Standard Deviation')
ax1.grid(True)
#ax1.hist(single_list_short)
#ax1.hist(single_sweep_list)
#ax1.set(title=ax1_title, xlabel=str('Collector Angle: ' + str(single_col_ang)), ylabel='Count')

#ax2 = fig1.add_subplot(1,2,2)
#ax2.hist(single_list_long)
#ax2.set(title='All Datasets', xlabel=str('Collector Angle: ' + str(single_col_ang)), ylabel='Count')
