...
mu_mass_ratio = mass_AC/mass_sun
mu_P2 = mu_P1 * mu_mass_ratio
r_target = ((r_origin**3)*mu_mass_ratio)**(1/3)  # Estimated from same density of Sun
...
total_time_years1 = 15
total_time_secs1 = total_time_years1*31536000
tau_f = total_time_secs1 * ((mu_P1/r_origin**3)**.5)

total_time_years2 = 2
total_time_secs2 = total_time_years2*31536000
tau_g = total_time_secs2 * ((mu_P2/r_target**3)**.5)
...
tot_x_len2 = int(np.ceil(tau_g/h))
...
for nu in nu_list:
    ...
    while x < tau_f-h and not transition_reached:
        ii += 1
        ...
        if counter == 10 and not predicted:
            predict = (100/60)*sum(np.diff(time_list))/len(time_list)
            print('------ Predicted Total Runtime: '+str(np.round(predict, decimals=2))+' min ------')
            predicted = True
        ...
        radius_in_AU = y[1]*r_origin/km_in_AU
        if radius_in_AU > transition_point_AU:
    ...
    num_save_total2 = 1000-1
    num_save_counter2 = 0
    num_save_index2 = 0

    rho_list2 = np.zeros(int(np.floor(tot_x_len2/num_save_total2)))
    theta_list2 = np.zeros_like(rho_list2)
    tau_list2 = np.zeros_like(rho_list2)

    # Carry over initial conditions from previous while loop
    rho_list2[0] = rho_list[-1]
    theta_list2[0] = theta_list2[-1]
    tau_list2[0] = tau_list[-1]

    #y = [rho_d, rho, theta_d, theta]
    #x = tau_i  # Declare time start
    #yrow = y  # Defining values
    ii2 = 0  # define iterations
    nn2 = 0  # define number of time steps               how is this different from ii?


    # Loop for 2nd planet thrusting time
    if transition_reached:
        while x < tau_f+tau_g-h:
            ...
            # Add to time step h
            nn2 += 1  #
            x = tau_i+h*nn2  # update time to represent steps marched so far

            #counter += 1
            percent2 = (((x-tau_f)/tau_g)*100)/3 + num_complete2*complete_perc

            #if counter >= progress_interval:
            if percent2 >= counter2:
                end = time.time()
                #print('Progress:', np.round(percent, decimals=2), '%   Runtime:',
                #      np.round((end-start)/60, decimals=2), 'min')
                print('2nd Planet Progress: '+str(counter2)+'%  ---  Runtime:', np.round((end-start)/60, decimals=2), 'min')
                counter2 += 1
            ...
            num_save_counter2 += 1
            if num_save_counter2 >= num_save_total:
                rho_list2[num_save_index2] = y[1]
                theta_list2[num_save_index2] = y[3]
                tau_list2[num_save_index2] = x
...
if transition_reached:
    total_time_years = total_time_years1+total_time_years2
else:
    total_time_years = total_time_years1
...
print('Gravitational Transition Reached:', str(transition_reached))
if transition_reached:
    print('Mean Angle at Transition:')
