...
#from E651_func_v1 import _1gauss,_1lorentz,_1voigt,_2gauss,_2lorentz,_2voigt,_2voigt_2cen
...
def _2voigt_skew_cen_mod2(arr_in, mas_cen1, mas_cen2, aG,cG,ampG,sG, aL,ampL,wL):
    output = []
    # CONVERT TO POLAR?

    output1 = _2gauss_skew()
    
#    if mas_cen1 == 0 and mas_cen2 == 0:
#        output1 = _2gauss_skew(x_list,   aG,cG,ampG,sG)
#        output2 = _2lorentz_skew(x_list, aL,cG,ampL,wL)  # Uses same center, cG
#    else:
#        x_list_mod = []
...
if fit_3d:
    ...
    for a in range(2*center_steps):  # change to while loop?
        ...
        rad_amp_dict = dict(zip(amplitude_list.copy(), radius_list.copy()))  # map(lambda i,j : (i,j) , amplitude_list,radius_list))
